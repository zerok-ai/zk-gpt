...
class Config:
    ...
    def fetch_secrets_with_retry(self, max_retries=5, base_timeout=1):
        retries = 0
        while retries < max_retries:
            secrets = self.fetch_secrets_from_client_service()
            if secrets:
                self.secrets = secrets  # Store the secrets in the class instance
                break
